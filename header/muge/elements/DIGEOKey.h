...
#ifndef __muge__elements__Key_h__
#define __muge__elements__Key_h__
...
namespace muge
{
	namespace elements
	{
		class DIGEOKey: public DIGEOInput
		{
		public:
			int code;
			enum EKEY_CODE
			...
			DIGEOKey(const char* tagName, int index = 0, Node* node = NULL):DIGEOInput(tagName, index, node)  
			{
			...
			void linkage()
			{				
				code = atoi(getAttributeByKey(DIGEOSintax::DIGEO_LanAttr_code));
				DIGEOInput::linkage();
			}
			...
			{
				if(Kernel::getInstance()->receiver->keyDown(key))
				{
